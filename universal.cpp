					...
					if (m_PlayerController->AcknowledgedPawn != nullptr)
					{
						...
						if (GetAsyncKeyState(VK_XBUTTON2) & 0x8000)
						{
							...
							if (closestPlayer != nullptr)
							{
								...
								Utils::Engine::GetBoneLocation(static_cast<SDK::ACharacter*>(closestPlayer)->Mesh, &playerLoc, 64);
						...
						if (m_PlayerController->AcknowledgedPawn->IsA(SDK::AFortPawn::StaticClass()))
						{
							...
							auto currentWeapon = static_cast<SDK::AFortWeapon*>(m_LocalPawn->CurrentWeapon);
...
HRESULT __stdcall hookD3D11Present(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
	...
	if (!psGreen)
	GenerateShader(pDevice, &psGreen, 0.0f, 1.0f, 1.0f);
	...
	if (pFontWrapper)
	{
		pFontWrapper->DrawString(pContext, L"MDRZN aimbot chams", 14, 16.0f, 16.0f, 0xff0000ff, FW1_RESTORESTATE);
